...
const init = function *init(themeName = 'peapod', ignore = [], themeReq, req) {
    ...
    /*
    const componentNameKeysOrig = Object.keys(componentNames);
    const componentNameKeysSorted = {};

    for (let componentNameIndex = 0, componentNameLen = componentNameKeysOrig.length; componentNameIndex < componentNameLen; componentNameIndex++) {
        const splitName = componentNameKeysOrig[componentNameIndex].split('_').length;

        if (typeof(componentNameKeysSorted[splitName]) === 'undefined') {
            componentNameKeysSorted[splitName] = [];
        }
        componentNameKeysSorted[splitName].push(componentNameKeysOrig[componentNameIndex]);
    }

    const componentNameKeys = [];

    const componentNameKeysSortedKeys = Object.keys(componentNameKeysSorted);
    for (let componentKeyIndex = 0, componentKeyLen = componentNameKeysSortedKeys.length; componentKeyIndex < componentKeyLen; componentKeyIndex++) {
        const arrayOfComponents = componentNameKeysSorted[componentNameKeysSortedKeys[componentKeyLen - componentKeyIndex - 1]];
        for (let i = 0, len = arrayOfComponents.length; i < len; i++) {
            componentNameKeys.push(arrayOfComponents[i]);
        }
    }
    */
